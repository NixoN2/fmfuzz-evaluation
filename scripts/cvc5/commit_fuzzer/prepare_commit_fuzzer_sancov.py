#!/usr/bin/env python3
"""
Prepare Commit Fuzzer (Sancov-enabled version)
Gets changed functions from a commit and finds tests that cover those functions.
Prepares a matrix for fuzzing jobs.
This version also extracts mangled function names for sancov coverage allowlist.
"""
...
class FunctionInfo:
    ...
    file: str
    mangled_name: Optional[str] = None  # Mangled name for sancov allowlist
...
class PrepareCommitAnalyzer:
    ...
    def get_function_signature(self, cursor) -> Optional[str]:
        ...
        try:
            ...
            # Note: mangled_name is a property in libclang Python bindings
            # It may not exist for all functions (e.g., static functions with internal linkage)
    ...
    def get_commit_functions(self, commit_hash: str) -> tuple[List[str], List[str], Dict[str, FunctionInfo]]:
        ...
        Returns:
            (changed_functions, files_with_no_functions, function_info_map) where:
            ...
            - function_info_map: Dict mapping function signature to FunctionInfo (includes mangled names for sancov)
        """
        ...
        function_info_map: Dict[str, FunctionInfo] = {}  # Map signature -> FunctionInfo
        ...
        for file_path, changed_lines in changed_files_lines.items():
            ...
            for sig_key, f in selected.items():
                ...
                # Store FunctionInfo with mangled name for sancov allowlist
                function_info_map[mapping_entry] = f
                print(f"    Selected: {mapping_entry} (overlap=True, sig_changed=False)")

        return (changed_functions, files_with_no_functions, function_info_map)
    ...
    def analyze_commit_coverage(self, commit_hash: str, coverage_json_path: str) -> Dict:
        ...
        # Step 1: Get changed functions from commit (including mangled names for sancov)
        changed_functions, files_with_no_functions, function_info_map = self.get_commit_functions(commit_hash)
        ...
        if not changed_functions:
            ...
            if files_with_no_functions:
                ...
                return {
                    ...
                    'function_info_map': {k: {'signature': v.signature, 'file': v.file, 'start': v.start, 'end': v.end, 'mangled_name': v.mangled_name} for k, v in function_info_map.items()},
...
def main():
    ...
    parser.add_argument('--output-changed-functions', help='Output changed functions list to JSON file')
    ...
    # Save changed functions if requested
