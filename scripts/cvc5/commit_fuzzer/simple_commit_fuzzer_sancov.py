#!/usr/bin/env python3
"""
Simple Commit Fuzzer (Sancov-enabled version)
Runs typefuzz on tests with multiple solvers and tracks sancov coverage.
"""
...
class SimpleCommitFuzzer:
    ...
    def __init__(
        ...
        # Sancov coverage tracking (optional)
        self.enable_sancov = enable_sancov
        ...
        if self.enable_sancov:
            try:
                ...
                from sancov_coverage_tracker import SancovCoverageTracker
                self.coverage_dir.mkdir(parents=True, exist_ok=True)
                self.coverage_tracker = SancovCoverageTracker(
    ...
    def run(self):
        ...
        # Save sancov coverage report if enabled
        if self.enable_sancov and self.coverage_tracker and self.coverage_report:
            try:
                self.coverage_tracker.save_coverage(self.coverage_report)
                stats = self.coverage_tracker.get_coverage_stats()
                print()
                print("[Sancov] Coverage Report:")
                print(f"  Total PCs: {stats['total_pcs']}")
                print(f"  Tests tracked: {stats['tests_tracked']}")
                print(f"  Processed .sancov files: {stats['processed_files']}")
                print(f"  Coverage report saved to: {self.coverage_report}")
            except Exception as e:
                print(f"[Sancov] Error saving coverage report: {e}", file=sys.stderr)
...
def main():
    ...
    parser.add_argument(
        "--enable-sancov",
        action="store_true",
        help="Enable sancov coverage tracking (requires ASAN_OPTIONS to be set)",
    )
    parser.add_argument(
        "--coverage-dir",
        default="./coverage",
        help="Directory for .sancov files (default: ./coverage, only used with --enable-sancov)",
    )
    parser.add_argument(
        "--coverage-report",
        help="Path to save coverage report JSON (default: not saved, only used with --enable-sancov)",
    )
    ...
    if args.time_remaining is None and args.job_start_time is None:
        # Set time_remaining to fuzzing duration in seconds
    ...
    try:
        fuzzer = SimpleCommitFuzzer(
            ...
            job_id=args.job_id,
            enable_sancov=args.enable_sancov,
